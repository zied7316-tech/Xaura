...
import { 
  ShoppingCart, Plus, Edit, Trash2, AlertTriangle,
  Search, RefreshCw, DollarSign, History, Clock, User, CreditCard
} from 'lucide-react'
import { formatCurrency, formatDateTime } from '../../utils/helpers'
import toast from 'react-hot-toast'

const ProductsForSalePage = () => {
  ...
  const loadProducts = async () => {
    try {
      setLoading(true)
      const data = await inventoryService.getProductsForSale()
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate required fields for for_sale products
    if (!formData.sellingPrice || formData.sellingPrice <= 0) {
      toast.error('Selling price is required for products for sale')
      return
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage products available for sale with pricing and payment methods</p>
        </div>
          ...
          <Card>
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600 mt-1">{formatCurrency(stats.totalSalesValue || 0)}</p>
              </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Products for Sale ({filteredProducts.length})</CardTitle>
        </CardHeader>
        <CardContent>
          {filteredProducts.length === 0 ? (
            <div className="text-center py-12">
              <ShoppingCart className="mx-auto text-gray-400 mb-4" size={64} />
              <p className="text-gray-600">No products for sale found</p>
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm">
                          {product.workerCommission?.type === 'percentage' 
                            ? <Badge variant="default">{product.workerCommission.percentage || 0}%</Badge>
                            : product.workerCommission?.type === 'fixed'
                            ? <Badge variant="default">{formatCurrency(product.workerCommission.fixedAmount || 0)}</Badge>
      ...
      <Modal
        ...
        title={editingProduct ? 'Edit Product for Sale' : 'Add New Product for Sale'}
        size="lg"
      >
